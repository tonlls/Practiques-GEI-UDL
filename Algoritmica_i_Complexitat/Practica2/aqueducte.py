...
class Aqueducte:
	...
	class Column:
		def __init__(self,x,y,h):
			...
			self.height=h-y
		def cost(self,alpha):
			return alpha*self.height
	def __init__(self,points,h,alpha,beta):
		self.height=h
		self.alpha=alpha
		self.beta=beta
	...
	def calc_greedy():
		...
		pass
	def calc_dynamic(self,i=0,j=0,points=[],mem={}):
		# if i==3:
			# return its_posible_multi_pilar	
		pass
	def calc_backtracking():
		pass
class Utils:
	...
	def calc_radius(rects):
		...
		return rad
	def its_possible(height,points):
		rads=calc_radius(points)
		hig=0
